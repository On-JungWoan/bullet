...
export default function Login() {
  ...
  const getRefreshToken = async () => {
    let token = [];
    await AsyncStorage.getItem(RefreshTOKEN).then((value) => {
      if (value) {
        // refreshToken
        token = value;
      }
    ...
    await reissueAccessToken(token);
  };
  ...
  const reissueAccessToken = useCallback(async (token) => {
    ...
    try {
      await axios
        .post(`${URL}/api/token/refresh/`, data)
        .then(function (response) {
          AsyncStorage.setItem(AccessTOKEN, response.headers.authorization);
          reissueToken =response.headers.authorization;
          setLogin(true);
        });
    } catch (error) {
      alert(`ERROR ${error}`)
    ...
    await getSite(reissueToken)
    await getKeyword(reissueToken)
  ...
  const checkLogin = async () => {
    ...
    await axios
      ...
      .then(function (response) {
        ...
        // 사이트 저장
        setNewsSites(response.data.news[0]?.sites ? response.data.news[0].sites : []);
        setUniSites(response.data.announce[0]?.sites ? response.data.announce[0].sites: []);
        setWorkSites(response.data.job[0]?.sites ? response.data.job[0].sites : []);

        // 키워드 저장
        setNewsKeywords(response.data.news[1]?.keywords ? response.data.news[1].keywords : []);
        setUniKeywords(response.data.announce[1]?.keywords ? response.data.announce[1].keywords: []);
        setWorkKeywords(response.data.job[1]?.keywords ? response.data.job[1].keywords : []);
      ...
      <ScrollView style={{ ...styles.loginContainer }}>
        ...
        >
          ...
          <BouncyCheckbox
            ...
            unfillColor="#FFFFFF"
            text="비밀번호 보기"
            iconStyle={{ borderColor: "red", marginTop: 5, color: "black" }}
        ...
        <View style={{ ...styles.findContainer }}>
          <Pressable>
            <Text style={{ fontSize: 10, color: "#888" }}>비밀번호 찾기</Text>
          </Pressable>
        </View>

        <View style={{ ...styles.socialLoginContainer }}>
          <Pressable style={{ ...styles.socialLogin }} onPress={()=>{alert("곧 업데이트")}}>
            <Image
              style={{ ...styles.img }}
          ...
          <Pressable style={{ ...styles.socialLogin }} onPress={()=>{alert("곧 업데이트")}}>
            <Image
              style={{ ...styles.img }}
              source={require("../../assets/social/naver.png")}
            />
            <Text>네이버</Text>
          </Pressable>
          <Pressable style={{ ...styles.socialLogin }} onPress={()=>{alert("곧 업데이트")}}>
